...
Keymaster's Keep is a dynamic, multi-game objective/challenge generator that seamlessly integrates with a thematic Archipelago randomizer. In other words, you have a typical Archipelago randomizer with goals, finite locations, progression items, logic etc., and Keymaster's Keep will generate and associate unique game challenges with each location. The APWorld provides increasing monolithic support for games, as well as a dedicated client to assist with the exploration and challenges of Keymaster's Keep.
...
![KMK Keymaster's Keep tab](images/keep.png)
